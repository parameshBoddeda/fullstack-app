...
import { Button, Dialog, DialogActions, DialogContent, Fab, IconButton, MenuItem, Select, TableFooter, TextField, Typography } from '@mui/material';
...
const AssetsList = ({ users }) => {
  ...
  React.useEffect(() => {
    axios.get('http://localhost:8000/getassets')
      .then((res) => {
        setData(res.data);
        setAdd(false);
        setDefined(true)
  ...
  React.useMemo(() => {
    if (initial.current) {
      axios.get(`http://localhost:8000/getuserassets?username=${user}`)
        .then((res) => {
  ...
  const handleAddAssets = () => {
    axios({
      method: 'post',
      url: 'http://localhost:8000/createassets',
      data: {
  ...
  const handleDone = (i) => {
    ...
    axios({
      method: 'put',
      url: 'http://localhost:8000/updateassets',
      data: {
  ...
  const handleRemove = () => {
    axios({
      method: 'put',
      url: 'http://localhost:8000/removeassets',
      data: {
        oldKeyboard: oldAssets.oldKeyboard,
        oldMouse: oldAssets.oldMouse,
        oldHeadphone: oldAssets.oldHeadphone,
  ...
  const handleSort = (e) => {
    ...
    if (e.target.value === 'asc') {
      setDetailedData({
        laptop: detailedData.laptop.sort((a, b) => a.model.localeCompare(b.model)),
        keyboard: detailedData.keyboard.sort((a, b) => a.model.localeCompare(b.model)),
        mouse: detailedData.mouse.sort((a, b) => a.model.localeCompare(b.model)),
        headphone: detailedData.headphone.sort((a, b) => a.model.localeCompare(b.model)),
        dongle: detailedData.dongle.sort((a, b) => a.model.localeCompare(b.model))
      })
      
    }
    else if(e.target.value === 'desc'){
      setDetailedData({
        laptop: detailedData.laptop.sort((a, b) => b.model.localeCompare(a.model)),
        keyboard: detailedData.keyboard.sort((a, b) => b.model.localeCompare(a.model)),
        mouse: detailedData.mouse.sort((a, b) => b.model.localeCompare(a.model)),
        headphone: detailedData.headphone.sort((a, b) => b.model.localeCompare(a.model)),
        dongle: detailedData.dongle.sort((a, b) => b.model.localeCompare(a.model))
      })
    }
    else{
      setDetailedData({
              ...
              <Table>
                ...
                <TableBody>
                  <StyledTableRow>
                    ...
                    <StyledTableCell>{userAssets[0]?.laptop ? userAssets[0]?.laptop : 'N/A'}</StyledTableCell>
                    <StyledTableCell>{userAssets[0]?.keyboard ? userAssets[0]?.keyboard : 'N/A'}</StyledTableCell>
                    <StyledTableCell>{userAssets[0]?.mouse ? userAssets[0]?.mouse : 'N/A'}</StyledTableCell>
                    <StyledTableCell>{userAssets[0]?.headphone ? userAssets[0]?.headphone : 'N/A'}</StyledTableCell>
                    <StyledTableCell>{userAssets[0]?.dongle ? userAssets[0]?.dongle : 'N/A'}</StyledTableCell>
                  </StyledTableRow>
                </TableBody>
              </Table>
            </TableContainer>
