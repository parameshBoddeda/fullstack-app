...
const UserForm = ({ setKey, username, setUsers, newUser }) => {
    ...
    React.useEffect(() => {
        axios.get(`http://localhost:8000/edituser?username=${username}`)
            .then((res) => {
                setUserdata({
    ...
    const handleUpdate = () => {
        updateSchema.validate(userdata, { abortEarly: false })
            .then((res) => {
                axios({
                    method: 'put',
                    url: `http://localhost:8000/updateuser?username=${username}`,
                    data: res
                })
                    .then((res) => {
                        setMessage(res.data)
    ...
    React.useMemo(()=>{
       if(initial.current){
        axios({
            method: 'put',
            url: `http://localhost:8000/enableuser?username=${username}`,
